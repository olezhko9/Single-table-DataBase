...
int DataBase::updateWhere(string field, string value, string newValue) {
	...
	// если нет записей, которые можно помен€ть, то выходим
	if (lines.size() == 0) {
		cout << "“акого пол€ не существует." << endl;
		return 0;
	}
		...
		for (int i = 0; i < lines.size(); i++) {
			// удал€ем каждую старую запись
			deleteRecord(lines[i].first);
			
			// измен€ем значение пол€
...
void DataBase::deleteRecord(int line) {
	...
	// пуста€ запись, котора€ заменит удал€емую
	string emptyRecord;
...
void DataBase::dropTable(string table) {
	// здесь должен быть код удалени€ папки по ее имени...
}
...
// ’эш функци€ с сайта http://qaru.site/questions/323303/hash-function-for-a-string
